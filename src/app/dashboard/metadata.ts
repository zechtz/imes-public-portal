...
export const getIdNameFields = ({ rename }) => [
  'id', `${rename ? 'displayName~rename(name)' : 'name,displayName'}`,
];
...
export const getFavoritesFields = ({ withDimensions, withOptions }) => [
  `reportTable[${getFavoriteFields({ withDimensions, withOptions }).join(',')}]`,
  `chart[${['type', ...getFavoriteFields({ withDimensions, withOptions })].join(',')}]`,
  `map[${getFavoriteFields({ withDimensions, withOptions }).join(',')}]`,
  `eventReport[${getFavoriteFields({ withDimensions, withOptions }).join(',')}]`,
  `eventChart[${getFavoriteFields({ withDimensions, withOptions }).join(',')}]`,
];
